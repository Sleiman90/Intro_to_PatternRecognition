...
path = r'C:\Users\HP\Desktop\FAU 1\Intro ML\introml_ex5_\introml_ex5\eigenfaces\*.png'
...
a,b,c=create_database_from_folder(path)
print(c)
...
def process_and_train(labels, train, num_images, h, w):
    ...
    # Calculate the maximum number of eigenfaces
    max_num_eigenfaces = min(num_images - 1, h * w)
    ...
    features = get_feature_representation(train, eigenfaces, avg, max_num_eigenfaces)
...
def calculate_eigenfaces(train, avg, num_eigenfaces, h, w):
    ...
    # Subtract the average face from every training sample
    zero_mean_train = train - avg[:, np.newaxis]

    # Compute the eigenfaces using SVD
    _, _, eigenfaces = np.linalg.svd(zero_mean_train.T, full_matrices=False)

    # Reshape the eigenfaces to match the image dimensions
    eigenfaces = eigenfaces.T.reshape((num_eigenfaces, h * w))
...
def get_feature_representation(images, eigenfaces, avg, num_eigenfaces):
    ...
    # Subtract the average face from every image
    zero_mean_images = images - avg[:, np.newaxis]
    
    # Project the zero-mean images onto the eigenfaces to compute the coefficients
...
def reconstruct_image(img, eigenfaces, avg, num_eigenfaces, h, w):
    ...
    coefficients = get_feature_representation(img_reshape[np.newaxis, :], eigenfaces, avg, num_eigenfaces)
...
def classify_image(img, eigenfaces, avg, num_eigenfaces, h, w):
    ...
    # Reshape the reconstructed image to match the input format of the classifier
    reconstructed_img_reshape = reconstructed_img.reshape((1, h * w))

    # Predict the label of the given image by feeding its coefficients to the classifier
    predicted_labels = clf.predict(reconstructed_img_reshape)
